...
F-01〜F-10 全機能を本番環境で動作させた Playwright 録画（約1分半）。

[fitlog-demo.webm](docs/demo/fitlog-demo.webm)

| 時間帯 | 内容 |
|--------|------|
| 冒頭 | F-01: ユーザー新規登録・ログイン |
| 〜30s | F-02/F-03: ベンチプレス3セット＋スクワット記録・保存 |
| 〜50s | F-09: 記録詳細で「自己ベスト更新🏅」バッジ確認 |
| 〜1分 | F-10: トレーニング写真アップロード |
| 〜1分10s | F-07: 週間4回の目標設定 |
| 〜1分20s | F-08: ダッシュボード（ストリーク・ヒートマップ・達成率）確認 |
| 〜1分30s | F-03: 記録編集 |
| 〜2分 | F-06: 鈴木ユーザー登録・田中をフォロー・フィード確認 |
| 〜2分20s | F-04: フィードでナイストレ |
| 〜2分30s | F-05: 詳細ページでアドバイス送信 |
| 終盤 | 田中で再ログイン → ナイストレ数・アドバイス反映を確認 |
...
### ダッシュボード（ストリーク・ヒートマップ・自己ベスト）

![ダッシュボード](docs/screenshots/dashboard.png)

### 記録入力

![記録入力](docs/screenshots/record_input.png)

### 記録詳細（自己ベスト更新🏅）

![記録詳細](docs/screenshots/record_detail.png)
...
インターネット
    │
CloudFront ─── S3（フロントエンド静的ファイル配信）
    │              S3（写真ストレージ）
    │
ALB（Application Load Balancer）
    │
ECS Fargate（バックエンド: FastAPI / Python）
    │
RDS（PostgreSQL 17）
```
...
| リソース | 用途 |
| -------- | ---- |
| CloudFront | HTTPS 終端・フロントエンド/API の振り分け（`/api/*` → ALB、`/*` → S3） |
| S3（frontend） | Vue 3 ビルド成果物の静的配信（OAC） |
| S3（media） | ユーザーアップロード写真の保存（OAC） |
| ALB | ECS へのリクエスト転送・ヘルスチェック |
| ECS Fargate | FastAPI アプリ（コンテナ起動時に Alembic マイグレーション実行） |
| RDS PostgreSQL 17 | データ永続化（プライベートサブネット配置） |
| ECR | バックエンド Docker イメージ管理 |
| Secrets Manager | DB パスワード・JWT シークレットの安全な管理 |
| GitHub Actions OIDC | アクセスキー不要の CI/CD デプロイ |
...
push to main
    ├── CI（並列）
    │   ├── pytest（バックエンド + PostgreSQL サービスコンテナ）
    │   └── Vitest + vue-tsc + vite build（フロントエンド）
    │
    └── Deploy to AWS（並列）
        ├── Docker build → ECR push → ECS force-deploy
