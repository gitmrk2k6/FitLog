...
def test_search_requires_auth(client):
    assert client.get("/users/search?q=a").status_code == 401


def test_follow_nonexistent_user_404(client, auth_headers):
    assert (
        client.post("/users/999999/follow", headers=auth_headers).status_code
        == 404
    )
...
def test_cannot_follow_self_422(client, auth_headers):
    ...
    res = client.post(f"/users/{me}/follow", headers=auth_headers)
...
def test_follow_returns_state(client, auth_headers):
    ...
    res = client.post(f"/users/{alice_id}/follow", headers=auth_headers)
    assert res.status_code == 201
...
def test_duplicate_follow_409(client, auth_headers):
    ...
    assert (
        client.post(f"/users/{aid}/follow", headers=auth_headers).status_code
        == 409
    )
...
def test_unfollow_then_not_following_404(client, auth_headers):
    ...
    client.post(f"/users/{aid}/follow", headers=auth_headers)
    assert (
        client.delete(f"/users/{aid}/follow", headers=auth_headers).status_code
        == 204
    )
    assert (
        client.delete(f"/users/{aid}/follow", headers=auth_headers).status_code
...
def test_search_excludes_self_and_shows_state(client, auth_headers):
    ...
    res = client.get("/users/search?q=alic", headers=auth_headers)
    ...
    client.post(f"/users/{aid}/follow", headers=auth_headers)
    again = client.get("/users/search?q=alice", headers=auth_headers).json()
...
def test_search_excludes_caller(client, auth_headers):
    ...
    res = client.get("/users/search?q=owner", headers=auth_headers)
    assert res.status_code == 200
    assert res.json() == []
...
def test_profile_counts_and_flags(client, auth_headers):
    ...
    prof = client.get(f"/users/{aid}", headers=auth_headers).json()
    ...
    own = client.get(f"/users/{owner_id}", headers=auth_headers).json()
...
def test_following_and_followers_lists(client, auth_headers):
    ...
    following = client.get(
        f"/users/{owner_id}/following", headers=auth_headers
    ).json()
    assert [u["username"] for u in following] == ["alice"]

    followers = client.get(
        f"/users/{aid}/followers", headers=auth_headers
    ).json()
...
def test_profile_nonexistent_404(client, auth_headers):
    assert (
        client.get("/users/999999", headers=auth_headers).status_code == 404
    )
...
def test_detail_visible_only_to_owner_or_follower(
    ...
    # 本人は見れる
    assert (
        client.get(f"/workouts/{wid}", headers=alice).status_code == 200
    )
    # 未フォロワーは 403
    assert (
        client.get(f"/workouts/{wid}", headers=other_headers).status_code
        == 403
    )
    # フォローすると見れる
    client.post(f"/users/{aid}/follow", headers=auth_headers)
    assert (
        client.get(f"/workouts/{wid}", headers=auth_headers).status_code
        == 200
    )
    # 存在しない記録は 404
    assert (
        client.get("/workouts/999999", headers=auth_headers).status_code
        == 404
    )
