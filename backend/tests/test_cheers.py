...
def test_cheer_requires_auth(client, auth_headers, ex_ids):
    ...
    assert client.post(f"/workouts/{wid}/cheers").status_code == 401


def test_cheer_nonexistent_workout_404(client, other_headers):
    res = client.post("/workouts/999999/cheers", headers=other_headers)
    assert res.status_code == 404


def test_cannot_cheer_own_workout_403(client, auth_headers, ex_ids):
    wid = _create_workout(client, auth_headers, ex_ids[0])
    res = client.post(f"/workouts/{wid}/cheers", headers=auth_headers)
...
def test_cheer_other_workout_returns_state(
    ...
    wid = _create_workout(client, auth_headers, ex_ids[0])
    res = client.post(f"/workouts/{wid}/cheers", headers=other_headers)
    assert res.status_code == 201
...
def test_duplicate_cheer_conflict_409(
    ...
    wid = _create_workout(client, auth_headers, ex_ids[0])
    client.post(f"/workouts/{wid}/cheers", headers=other_headers)
    dup = client.post(f"/workouts/{wid}/cheers", headers=other_headers)
...
def test_uncheer_decrements_count(
    ...
    wid = _create_workout(client, auth_headers, ex_ids[0])
    client.post(f"/workouts/{wid}/cheers", headers=other_headers)
    res = client.delete(f"/workouts/{wid}/cheers", headers=other_headers)
